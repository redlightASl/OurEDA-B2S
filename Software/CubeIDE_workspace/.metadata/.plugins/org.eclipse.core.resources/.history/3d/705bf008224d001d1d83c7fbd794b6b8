...
/**
 * @brief
 * @param  Pid              MyParamdoc
 * @param  target           MyParamdoc
...
float_t calculatePID(Algorithm_PID_t Pid, float_t target, float_t value)
{
    ...
    Pid->Output = Pid->Kp * Pid->Error + Pid->Ki * Pid->DeltaIntegral + Pid->Kd * (Pid->Error - Pid->LastError); //PID

    if (Pid->Output < -100) //restrict output
    {
        Pid->Output = -100;
    }
 ...
 */
static uint8_t IdTest(uint8_t* String, uint8_t Method, uint8_t StringLength, uint8_t IdBitAddr)
{
#ifdef DATA_IDENTIFY
    switch (Method)
    {
